...
`dtos`, `model`, `service`, `service/appointment` e `controller/user`.
...
- A conversão de `Appointment` é feita explicitamente pelo `AppointmentService`, preservando snapshots de duração/preço e vínculos de unidade, serviço, profissional, série e remarcação.
